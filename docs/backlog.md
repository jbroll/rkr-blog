...
Build hygiene, security, video, and the WordPress permalink redirect are done, along with group 4's sync and PWA items. What remains is the three editor/figure features in group 4, the canvas coverage gate (blocked — see `DEFERRED.md`), the dump converter's binary literals (group 5), and the long-tail items in groups 6-8.
...
- [ ] **Package canvas layer not c8-gated** — `packages/image-edit/src/canvas/**` excluded like `src/admin`; only `src/core` gated. Blocked upstream: monocart never offers those sources to the e2e coverage hooks. (`DEFERRED.md: image-pwa`)
