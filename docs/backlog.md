...
- [ ] **`check-bundle-size.ts --write` not implemented** — error message tells caller to re-run with `--write` but `argv` is never parsed. Implement the flag or drop the hint. (`DEFERRED.md: Performance`)
