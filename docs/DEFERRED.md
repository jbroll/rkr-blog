...
- **`resolveSavedStatus`/`resolveSavedDate` vanish window** (`src/routes/admin-frontmatter.ts:14`, `:30`) — `existsSync` then `readFileSync`; a file deleted in between throws ENOENT. _Revisit when:_ this crash is observed, or the vanish-window class gets fixed elsewhere and this one should match.
...
- **`scripts/walk-site.sh` pagination never terminates** — it treats every
  root-relative `href` as a post slug, so chrome links (`/about`, `/login`)
