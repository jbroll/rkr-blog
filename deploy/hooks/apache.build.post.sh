...
# ${APP_DIR}/image-editor by fastify_app.build.post.sh under the same opt-in.
# Served directly by Apache (the '!' excludes it from the proxy). Must precede
# ProxyPass /.
IMAGE_EDITOR_BLOCK=""
if [[ "${DEPLOY_IMAGE_EDITOR:-no}" == "yes" ]]; then
  IMAGE_EDITOR_BLOCK="    Alias /image-editor ${APP_DIR}/image-editor
