...
- Single password, argon2id, stored in the `auth` table.
- Set/changed via `site-admin password` (interactive prompt).
- Session cookie: `HttpOnly`, `Secure`, `SameSite=Lax`, 32-byte random ID, server-side session table, 30-day expiry.
- Login rate-limit: 5 attempts per 15 min per IP via Fastify rate-limit plugin.
- No password reset flow; recovery is `site-admin password` with shell access.
- No CSRF protection in v1 — single-author, `SameSite=Lax` session cookie, no third-party-initiated state changes. Revisit if any admin route accepts cross-origin requests.
