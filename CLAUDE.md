...
See `docs/developer-quickstart.md §4` for the project's coding conventions
...
- Deferred work goes in `docs/DEFERRED.md`, not in commit messages or
  scattered TODOs. Keep it terse: one line per item, grouped by area,
  with a `_revisit when:_ <trigger>`. No Source/What/Why prose — the
  rationale lives in the commit/spec that introduced it. Delete an
  item when it ships.
