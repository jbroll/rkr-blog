...
export async function buildApp(opts: BuildAppOpts = {}): Promise<FastifyInstance> {
  ...
  //
  // Discloses ADMIN_TOKEN to anyone who knows the path. Re-added (this
  // is the second time — see git history for the prior 72725aa /
  // ab2a626 cycle) to diagnose a token-mismatch on rkr-blog.fly.dev
  // where the operator can't reach fly's console on mobile and the
  // value in local secret.env doesn't match what fly is running.
  // Tracked in DEFERRED.md under "Remove /_debug/admin-token". Do
  // NOT ship to anything other than a throwaway dev demo.
