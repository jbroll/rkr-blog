...
export function parsePost(raw: string): ParsedPost {
  ...
  // Frontmatter scalars are literal post-YAML-parse. We deliberately do
  // NOT HTML-entity-decode title/subtitle here: decoding would put live
  // `<`/`>`/`"` into the in-memory title, turning any future raw-title
  // sink (meta/OpenGraph/RSS/JSON-LD) into an injection point. WP imports
  // arrive entity-encoded and are decoded once, in the WP importer.
